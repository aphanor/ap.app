var BV = angular.module('bvapp', ['angular-loading-bar', 'ngAnimate', 'ui.bootstrap']);

BV.controller('bvapi', ['$scope', '$http', '$window', '$q', '$rootScope', function($scope, $http, $window, $q, $rootScope) {
            ...
            $http.get($scope.api_URL).then(function(response) { 
                ...
                if ($scope.api_URL.indexOf("reviews.json") != -1) {
                    ...
                    $scope.ratingsOnly = $scope.apidata.IsRatingsOnly;
                        ...
                        /*$scope.pTitle = $scope.apidata.Includes.Object.keys(Products)[0].Name;
                        $scope.pDescription = $scope.apidata.Includes.Object.keys(Products)[0].Description;
                        $scope.pImage = $scope.apidata.Includes.Object.keys(Products)[0].ImageUrl;
                        $scope.pURL = $scope.apidata.Includes.Object.keys(Products)[0].ProductPageUrl; */
                    ...
                    console.log("Page: " + $scope.page + " - Limit: " + $scope.limit);
                    $scope.p = {};
                    $scope.pager = [];
                    ...
                    if($scope.page != Math.floor($scope.page)) {
                        ...
                        for (i = 0; i < $scope.page.length; i++) { 
                            if(i == 0) {
                                $http.get($scope.api_URL).then(function(b) {
                                    //$scope.p.push(b.data.Results);
                                    //$scope.p = JSON.stringify(b.data.Results);
                                    $scope.api_URL = '"' + $scope.api_URL + '"';
                                    $scope.pager = { url : + $scope.api_URL };
                                    console.log(a);
                                });
                            } else {
                                $scope.Offset = i * $scope.limit;
                                console.log("Offset: " + $scope.Offset);
                                $http.get($scope.api_URL + "&offset=" + $scope.Offset).then(function(y) {
                                    
                                    //$scope.addP.push(y.data.Results);
                                    //$scope.p.push.apply($scope.p, $scope.addP);
                                    //$scope.p[i] = JSON.stringify(y.data.Results);
                                    //console.log($scope.p[i]);
                                    var UrlWithOffset = '"' + $scope.api_URL + "&offset=" + $scope.Offset + '"';
                                    var pageD = { url : + UrlWithOffset };
                                    $scope.pager.push(pageD);
